...
const ALPM_BASE: &str = "/tmp/alpmtest";
const ALPM_DB: &str = "/tmp/alpmtest/var/lib/pacman";
...
fn main() {
    ...
    println!(
        "Creating Alpm instance with base \"{}\" and db \"{}\"",
        ALPM_BASE, ALPM_DB
    );
    let alpm = Alpm::new(ALPM_BASE, ALPM_DB).unwrap();
    ...
    transaction
        .add_package(alpm.sync_dbs()[0].pkg("tzdata").unwrap())
        .unwrap();
